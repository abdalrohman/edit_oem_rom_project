# this file is part of https://github.com/cubinator/ext4
...
class ext4_inode (ext4_struct):
  ...
  _fields_ = [
    ...
    # 0x006C, Originally named i_size_high
...
class Inode:
  ...
  def _parse_xattrs(self, raw_data, offset, prefix_override={}):
    ...
    while i < len(raw_data):
      ...
      if xattr_entry.e_value_inum != 0:
        ...
        # internal xattr
        xattr_value = raw_data[xattr_entry.e_value_offs +
                               offset: xattr_entry.e_value_offs + offset + xattr_entry.e_value_size]
  ...
  def get_inode(self, *relative_path, decode_name=None):
    ...
    for i, part in enumerate(relative_path):
      if not self.volume.ignore_flags and not current_inode.is_dir:
        ...
        raise Ext4Error(
          f"{current_path!r:s} (Inode {inode_idx:d}) is not a directory.")

      file_name, inode_idx, file_type = next(filter(
        lambda entry: entry[0] == part, current_inode.open_dir(decode_name)), (None, None, None))
  ...
  def mode_str(self):
    ...
    return "".join([
      ...
      special_flag("s", (self.inode.i_mode & ext4_inode.S_IXUSR)
                   != 0, (self.inode.i_mode & ext4_inode.S_ISUID) != 0),

      "r" if (self.inode.i_mode & ext4_inode.S_IRGRP) != 0 else "-",
      "w" if (self.inode.i_mode & ext4_inode.S_IWGRP) != 0 else "-",
      special_flag("s", (self.inode.i_mode & ext4_inode.S_IXGRP)
                   != 0, (self.inode.i_mode & ext4_inode.S_ISGID) != 0),

      "r" if (self.inode.i_mode & ext4_inode.S_IROTH) != 0 else "-",
      "w" if (self.inode.i_mode & ext4_inode.S_IWOTH) != 0 else "-",
      special_flag("t", (self.inode.i_mode & ext4_inode.S_IXOTH)
                   != 0, (self.inode.i_mode & ext4_inode.S_ISVTX) != 0),
    ])
    ...
    if (self.inode.i_flags & ext4_inode.EXT4_EXTENTS_FL) != 0:
      ...
      while nodes.qsize() != 0:
        ...
        if not self.volume.ignore_magic and header.eh_magic != 0xF30A:
          raise MagicError(
            f"Invalid magic value in extent header at offset 0x{header_offset:X} of inode {self.inode_idx:d}: 0x{header.eh_magic:04X} (expected 0xF30A)")

        if header.eh_depth != 0:
          indices = self.volume.read_struct(
            ext4_extent_idx * header.eh_entries, header_offset + ctypes.sizeof(ext4_extent_header))
          for idx in indices:
            nodes.put_nowait(idx.ei_leaf * self.volume.block_size)
        else:
          extents = self.volume.read_struct(
            ext4_extent * header.eh_entries, header_offset + ctypes.sizeof(ext4_extent_header))
  ...
  def xattrs(self, check_inline=True, check_block=True, force_inline=False, prefix_override={}):
    ...
    if check_block and self.inode.i_file_acl != 0:
      ...
      if not self.volume.ignore_magic and xattrs_header.h_magic != 0xEA020000:
        raise MagicError(
          f"Invalid magic value in xattrs block header at offset 0x{xattrs_block_start:X} of inode {self.inode_idx:d}: 0x{xattrs_header.h_magic} (expected 0xEA020000)")
...
class BlockReader:
  ...
  def read(self, byte_len=-1):
    ...
    byte_len = (byte_len + start_offset - self.volume.block_size -
                1) % self.volume.block_size + 1
...
class Tools:
  ...
  def list_dir(
      ...
      line_format=None,
      file_types={0: "unkn", 1: "file", 2: "dir",
                  3: "chr", 4: "blk", 5: "fifo", 6: "sock", 7: "sym"}
  ):
    """
    ...
    # Print directory
    entries = inode.open_dir(decode_name) if sort_key is None else sorted(
      inode.open_dir(decode_name), key=sort_key)
